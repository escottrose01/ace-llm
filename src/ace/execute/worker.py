...
def display(data: str) -> None:
    ...
    writer_thread = threading.Thread(target=handle_write, args=(ORCHESTRATOR_ADDR, f"PRINT:{encoded}"), daemon=True)
    writer_thread.start()
...
def invoke(tool_name: str, *args: Any, **kwargs: Any) -> Any:
    ...
    writer_thread = threading.Thread(target=handle_write, args=(ORCHESTRATOR_ADDR, message), daemon=True)
    writer_thread.start()
...
def main() -> None:
    ...
    if not script.strip():
        msg_b64 = base64.b64encode(b"No script provided in $SCRIPT.").decode()
        writer_thread = threading.Thread(target=handle_write, args=(ORCHESTRATOR_ADDR, f"PRINT:{msg_b64}"), daemon=True)
        writer_thread.start()
        sys.exit(1)
    ...
    try:
        ...
        writer_thread = threading.Thread(
            target=handle_write, args=(ORCHESTRATOR_ADDR, f"PRINT:{e_msg_b64}"), daemon=True
        )
        writer_thread.start()
    ...
    writer_thread = threading.Thread(target=handle_write, args=(ORCHESTRATOR_ADDR, term_msg), daemon=True)
    writer_thread.start()
